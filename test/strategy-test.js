...
var JustintvStrategy = require('passport-justintv/strategy');


vows.describe('JustintvStrategy').addBatch({
  
  'strategy': {
    topic: function() {
      return new JustintvStrategy({
        consumerKey: 'ABC123',
    ...
    'should be named justintv': function (strategy) {
      assert.equal(strategy.name, 'justintv');
    },
  },
  
  'strategy when loading user profile': {
    topic: function() {
      var strategy = new JustintvStrategy({
        consumerKey: 'ABC123',
        consumerSecret: 'secret'
      },
      function() {});
    ...
    'when told to load user profile': {
      ...
      'should load profile' : function(err, profile) {
        assert.equal(profile.provider, 'justintv');
